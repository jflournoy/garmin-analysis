#!/usr/bin/env python3
"""Test optimized spline model with cov_exp_quad and sparse GP options."""
import sys
...
def test_optimized_model_full_gp():
    """Test that optimized model with full GP (use_sparse=False) works."""
    print("Testing optimized spline model (full GP)...")
    ...
    try:
        ...
        print("✓ Optimized spline model (full GP) fitted successfully")
        ...
        required_params = ['alpha_trend', 'rho_trend', 'sigma', 'a_sin', 'a_cos']
...
def test_numerical_equivalence():
    """Compare original and optimized full GP models for numerical equivalence."""
    print("\n" + "="*60)
    print("Testing numerical equivalence: original vs optimized (full GP)")
    print("="*60)
    ...
    print("Fitting original spline model...")
    try:
        fit_orig, idata_orig, df_orig, stan_data_orig = fit_weight_model_spline(
            chains=chains,
        ...
        print("✓ Original model fitted")
    except Exception as e:
        print(f"✗ Original model failed: {e}")
        return False

    print("Fitting optimized spline model (full GP)...")
    try:
        fit_opt, idata_opt, df_opt, stan_data_opt = fit_weight_model_spline_optimized(
            chains=chains,
            iter_warmup=iter_warmup,
            iter_sampling=iter_sampling,
            fourier_harmonics=fourier_harmonics,
    ...
    comparisons = []
    for param in ['alpha_trend', 'rho_trend', 'sigma']:
    ...
    if 'a_sin' in idata_orig.posterior and 'a_sin' in idata_opt.posterior:
        a_sin_orig = idata_orig.posterior['a_sin'].mean(dim=['chain', 'draw']).values
        a_sin_opt = idata_opt.posterior['a_sin'].mean(dim=['chain', 'draw']).values
        max_diff = np.max(np.abs(a_sin_orig - a_sin_opt))
        within_tol = max_diff <= tolerance
        comparisons.append(('a_sin', np.mean(a_sin_orig), np.mean(a_sin_opt), max_diff, within_tol))
        print(f"  a_sin: max diff={max_diff:.4f} {'✓' if within_tol else '✗'}")

    if 'a_cos' in idata_orig.posterior and 'a_cos' in idata_opt.posterior:
        a_cos_orig = idata_orig.posterior['a_cos'].mean(dim=['chain', 'draw']).values
        a_cos_opt = idata_opt.posterior['a_cos'].mean(dim=['chain', 'draw']).values
        max_diff = np.max(np.abs(a_cos_orig - a_cos_opt))
        within_tol = max_diff <= tolerance
        comparisons.append(('a_cos', np.mean(a_cos_orig), np.mean(a_cos_opt), max_diff, within_tol))
        print(f"  a_cos: max diff={max_diff:.4f} {'✓' if within_tol else '✗'}")
...
def main():
    """Run all tests."""
    print("="*70)
    print("OPTIMIZED SPLINE MODEL TEST SUITE")
    print("="*70)
