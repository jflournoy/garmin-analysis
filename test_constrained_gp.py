#!/usr/bin/env python3
"""Test the constrained GP four-fitness model."""
...
def test_constrained_gp():
    """Test the constrained GP model."""
    print("\n" + "=" * 70)
    print("TESTING CONSTRAINED GP FOUR-FITNESS MODEL")
    print("=" * 70)

    # Configuration
    output_dir = Path("output/four_fitness_constrained_gp")
    ...
    iter_warmup = 500
    iter_sampling = 500
    adapt_delta = 0.99  # Higher to reduce divergent transitions
    max_treedepth = 12
    fourier_harmonics = 2
    ...
    # Fit model
    print("\n" + "=" * 70)
    print("FITTING CONSTRAINED GP MODEL")
    print("=" * 70)
    ...
    print(f"  Adapt delta: {adapt_delta} (higher to reduce divergent transitions)")
    print(f"  Max treedepth: {max_treedepth}")
    print(f"  Inducing points: {n_inducing_points}")
    print(f"  Fourier harmonics: {fourier_harmonics}")

    # Compile model
    model_path = Path("stan/weight_state_space_four_fitness_constrained_gp.stan")
    ...
    # Print key results
    print("\n" + "=" * 70)
    print("KEY RESULTS - CONSTRAINED GP MODEL")
    print("=" * 70)
    ...
    if 'gamma_a_short' in idata.posterior:
        ...
        print(f"  Gamma aerobic short-term: {gamma_a_short:.3f} ± {gamma_a_short_sd:.3f} (should be negative ~-0.3)")
    ...
    if 'gamma_s_short' in idata.posterior:
        ...
        print(f"  Gamma strength short-term: {gamma_s_short:.3f} ± {gamma_s_short_sd:.3f} (should be positive ~0.2)")
    ...
    if 'gamma_a_long' in idata.posterior:
        ...
        print(f"  Gamma aerobic long-term: {gamma_a_long:.3f} ± {gamma_a_long_sd:.3f} (should be negative ~-0.2)")
    ...
    if 'gamma_s_long' in idata.posterior:
        ...
        print(f"  Gamma strength long-term: {gamma_s_long:.3f} ± {gamma_s_long_sd:.3f} (should be positive ~0.3)")

    # GP parameters
    if 'alpha_gp' in idata.posterior:
        alpha_gp = idata.posterior['alpha_gp'].values.mean()
        print(f"  GP standard deviation (alpha_gp): {alpha_gp:.3f} (constrained < 0.5)")

    if 'rho_gp' in idata.posterior:
        rho_gp = idata.posterior['rho_gp'].values.mean()
        print(f"  GP length scale (rho_gp): {rho_gp:.3f} (constrained 0.05-1.0)")
    ...
    print("=" * 70)
    print("Original model (weak priors):")
    print("  Fitness explains: ~4.2% of variance")
    print("  GP explains: 78.1% of variance")
    print("  Weight effects have wide uncertainty (include zero)")
    print("\nTightened model (no GP):")
    print("  Fitness explains: 91.6% of variance")
    print("  Weight effects precise and physiologically reasonable")
    print("\nConstrained GP model (informative priors):")
    print("  Should give intermediate results")
    print("  Fitness: 20-40% (hopefully)")
    print("  GP: 40-60% (hopefully)")
    print("  Weight effects: precise and reasonable")
    ...
    print(f"  1. Does fitness explain meaningful variance (20-40%)?")
    print(f"  2. Are weight effects physiologically reasonable?")
    print(f"  3. Is the GP appropriately constrained (alpha_gp < 0.5)?")
    print(f"\nDeeper issues (as you identified):")
    print(f"  1. Is the impulse-decay fitness model correct?")
    print(f"  2. Are HR-based intensity metrics the right predictors?")
    print(f"  3. Would actual volume lifted be better for strength training?")
