/*
 * Four-fitness state-space model for weight with separate short-term and long-term effects
 * for both strength and aerobic training.
 *
...
parameters {
  ...
  // Weight effects (key physiological parameters)
...
model {
  // Priors for impulse decay parameters
  // Short-term: faster decay (smaller values)
  psi_a_short ~ beta(3, 5);           // favors ~0.375 (faster decay)
  psi_s_short ~ beta(3, 5);           // favors ~0.375 (faster decay)

  // Long-term: slower decay (larger values)
  psi_a_long ~ beta(5, 2);            // favors ~0.714 (slower decay)
  psi_s_long ~ beta(5, 2);            // favors ~0.714 (slower decay)

  // Priors for fitness decay parameters
  // Short-term: faster fitness decay
  alpha_a_short ~ beta(4, 4);         // favors ~0.5 (moderate decay)
  alpha_s_short ~ beta(4, 4);         // favors ~0.5 (moderate decay)

  // Long-term: slower fitness decay (fitness persists)
  alpha_a_long ~ beta(8, 2);          // favors ~0.8 (slow decay)
  alpha_s_long ~ beta(9, 1);          // favors ~0.9 (very slow decay for muscle)

  // Priors for fitness gain parameters (positive only - workouts increase fitness)
  beta_a_short ~ exponential(5);      // mean = 0.2, mode = 0 (short-term aerobic gain)
  beta_s_short ~ exponential(5);      // mean = 0.2, mode = 0 (short-term strength gain)
  beta_a_long ~ exponential(10);      // mean = 0.1, mode = 0 (long-term aerobic gain)
  beta_s_long ~ exponential(10);      // mean = 0.1, mode = 0 (long-term strength gain)

  // KEY PRIORS: Weight effects based on physiology
  // Short-term aerobic: dehydration reduces weight
  gamma_a_short ~ normal(-0.3, 0.15);

  // Short-term strength: inflammation/water retention increases weight
  gamma_s_short ~ normal(0.2, 0.15);

  // Long-term aerobic: fat loss reduces weight
  gamma_a_long ~ normal(-0.2, 0.1);

  // Long-term strength: muscle gain increases weight
  gamma_s_long ~ normal(0.3, 0.15);
