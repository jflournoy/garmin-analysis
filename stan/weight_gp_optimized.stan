/*
 * OPTIMIZED Gaussian Process model for weight over time (original model)
 *
 * This model uses Stan's built-in gp_exp_quad_cov function for efficient
 * covariance computation, with optional sparse GP approximation.
 *
 * OPTIMIZATIONS:
...
model {
  ...
  alpha ~ normal(0, 0.5);          // GP amplitude
  rho ~ inv_gamma(5, 1);         // Length scale (fairly smooth)
  sigma ~ exponential(2);        // Observation noise
