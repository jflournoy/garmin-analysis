 ...
 * REGULARIZING PRIOR on AR(1) innovation scale to shrink AR component.
 * GENERATES PREDICTIONS FOR ALL DAYS AT MULTIPLE HOURS.
 *
 ...
 * Regularizing prior on sigma_epsilon to shrink AR component toward zero
 * Generates predictions for all days at multiple hours
 */
...
parameters {
  ...
  // AR(1) process parameters with regularizing prior
...
transformed parameters {
  ...
  // Add small constant to avoid division by zero when rho ≈ 1
  epsilon[1] = sigma_epsilon / sqrt(max(1 - rho^2, 1e-6)) * epsilon_raw[1];
...
model {
  ...
  // Regularizing prior for AR(1) coefficient - centered at 0 with small variance
  rho ~ normal(0, 0.2);                   // More informative to encourage lower autocorrelation

  // Regularizing prior for innovation standard deviation
  // Half-normal with small scale to shrink toward zero
  sigma_epsilon ~ normal(0, 0.1);         // Regularizing prior
