/*
 * State-space model with diminishing returns - FIXED SCALING.
 *
 * Key changes:
 * 1. Use Michaelis-Menten: gain = impulse / (1 + k * fitness)
 *    - Scale-invariant, bounded gain ≤ impulse
 *    - k has units of 1/fitness (easier to set prior)
 *
 * 2. Rescale intensity: divide by 1000 to get reasonable units
 *    - Original intensity = duration * HR_diff
 *    - Scaled intensity = intensity / 1000
 *
 * 3. Fix k based on example pattern
 *    - From example: fitness=2, gain=1.5 (75% of max)
 *    - 1/(1 + k*2) = 0.75 → k = (1/0.75 - 1)/2 = 0.1667
 *    - Set k ~ normal(0.17, 0.05)
 */
...
parameters {
  ...
  // Diminishing returns parameters (Michaelis-Menten)
  real<lower=0> k_a;                  // aerobic diminishing returns (1/fitness units)
...
transformed parameters {
  ...
  // Compute fitness states with Michaelis-Menten diminishing returns
  // Fixed beta = 1.0: impulse converts to fitness with diminishing returns
  fitness_a[1] = 0;
  fitness_s[1] = 0;

  for (t in 2:D) {
    // Michaelis-Menten: gain = impulse / (1 + k * current_fitness)
    // This ensures: gain ≤ impulse, and gain → 0 as fitness → ∞
    real gain_a = impulse_a[t-1] / (1 + k_a * fitness_a[t-1]);
    real gain_s = impulse_s[t-1] / (1 + k_s * fitness_s[t-1]);
...
model {
  ...
  // Priors for diminishing returns (based on example pattern)
  // Example: fitness=2, gain=1.5 (75% of max when fitness=0)
  // 1/(1 + k*2) = 0.75 → k = (1/0.75 - 1)/2 = 0.1667
  k_a ~ normal(0.17, 0.1);            // centered at example value
  k_s ~ normal(0.17, 0.1);

  // STRONGER priors for weight effects
  gamma_a ~ normal(-0.3, 0.1);        // aerobic fitness reduces weight
  gamma_s ~ normal(0.2, 0.1);         // strength might increase weight (muscle)
...
generated quantities {
  ...
  vector[D] gain_fraction_a;  // gain / impulse (0 to 1)
  vector[D] gain_fraction_s;
  ...
  for (t in 2:D) {
    effective_gain_a[t] = impulse_a[t-1] / (1 + k_a * fitness_a[t-1]);
    effective_gain_s[t] = impulse_s[t-1] / (1 + k_s * fitness_s[t-1]);

    if (impulse_a[t-1] > 0) {
      gain_fraction_a[t] = effective_gain_a[t] / impulse_a[t-1];
    } else {
      gain_fraction_a[t] = 0;
    }

    if (impulse_s[t-1] > 0) {
      gain_fraction_s[t] = effective_gain_s[t] / impulse_s[t-1];
    } else {
      gain_fraction_s[t] = 0;
    }
  }
