 ...
 * VERSION 2: Fixed parameter constraints
 *
 * Key improvements:
 ...
 * 4. Proper constraints on all parameters
...
parameters {
  ...
  real<lower=0.1, upper=0.5> alpha_gp;  // GP marginal std (0.1-0.5)
...
model {
  ...
  sigma_psi_short ~ exponential(5);    // mean=0.2

  mu_alpha_short ~ beta(4, 4);         // favors ~0.5 (moderate decay)
  sigma_alpha_short ~ exponential(5);

  mu_beta_short ~ exponential(10);     // mean=0.1
  sigma_beta_short ~ exponential(5);

  // Long-term hyperpriors
  mu_psi_long ~ beta(5, 2);            // favors ~0.71 (slow decay)
  sigma_psi_long ~ exponential(5);

  mu_alpha_long ~ beta(6, 2);          // favors ~0.75 (very slow decay)
  sigma_alpha_long ~ exponential(5);

  mu_beta_long ~ exponential(20);      // mean=0.05 (smaller gain)
  sigma_beta_long ~ exponential(5);
  ...
  // GP parameters
  alpha_gp ~ beta(2, 3);               // favors smaller values (mean ~0.4)
