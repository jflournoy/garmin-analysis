 ...
 * WITH INTERCEPT TERM and LINEAR TIME TREND in weight model.
 ...
 *
 * Weight model WITH INTERCEPT, LINEAR TREND, STUDENT-T LIKELIHOOD, AR(1) CORRELATION, and DAILY SPLINE:
 *   weight[t] ~ student_t(nu, mu[t], sigma_w)
 *   mu[t] = weight_intercept + delta * (t - 0.5*D) / D +
 *           gamma_s * strength_fitness[day(t)] +
 ...
 *   delta ~ normal(0, 1.0)  # Weakly informative: 1 std unit ≈ 2.77 lbs over full range
 *   Time index centered to reduce intercept-slope correlation
 *
 * Using Student-t distribution for robust regression (handles outliers better)
 * HEAVILY CONSTRAINED AR(1) process to prevent overfitting
...
parameters {
  ...
  real delta;                         // LINEAR TREND SLOPE (NEW!)
...
transformed parameters {
  ...
  // Compute linear predictors without AR component (includes daily spline and trend)
  for (i in 1:N_weight) {
    mu_no_ar[i] = weight_intercept +
                  delta * (day_idx[i] - 0.5 * D) / D +
...
model {
  ...
  // Prior for linear trend (NEW!)
  // Weakly informative: 1 std unit ≈ 2.77 lbs over full range
  // This corresponds to roughly 1.1 lbs/year for the 2.5-year window
  delta ~ normal(0, 1.0);
...
generated quantities {
  ...
  // Store AR(1) and trend parameters
  real sigma_epsilon_stored = sigma_epsilon;
  real rho_stored = rho;
  real delta_stored = delta;
  ...
  for (t in 1:D) {
    for (h in 1:H) {
      ...
      // Linear predictor without AR component (structural model only, includes trend)
