/*
 * Bivariate Gaussian Process model for weight and another variable with mismatched observation times.
 ...
 *
 * Allows different observation times and counts for each variable.
 */

data {
  // Weight variable observations
  int<lower=1> N_weight;          // Number of weight observations
  array[N_weight] real t_weight;  // Time points for weight (scaled to [0,1])
  vector[N_weight] y_weight;      // Weight observations (standardized)

  // Other variable observations
  int<lower=1> N_other;           // Number of other variable observations
  array[N_other] real t_other;    // Time points for other variable (scaled to [0,1])
  vector[N_other] y_other;        // Other variable observations (standardized)
  ...
  // Prediction grid (optional) - shared grid for both outputs
...
parameters {
  ...
  vector<lower=0.01>[2] sigma_B;             // Standard deviations for each output
...
transformed parameters {
  ...
  // Covariance between observed points and inducing points for each output
  matrix[N_weight, M] K_fu_weight = gp_exp_quad_cov(t_weight, t_inducing, alpha, rho);
  matrix[N_other, M] K_fu_other = gp_exp_quad_cov(t_other, t_inducing, alpha, rho);
  ...
  // Trend component at observed points for each output
  vector[N_weight] f_weight = K_fu_weight * a[:, 1];
  vector[N_other] f_other = K_fu_other * a[:, 2];
}
...
model {
  ...
  y_weight ~ student_t(nu[1], f_weight, sigma[1]);
  y_other ~ student_t(nu[2], f_other, sigma[2]);
}

generated quantities {
  // Posterior predictive
  vector[N_weight] y_weight_rep;
  vector[N_other] y_other_rep;

  // Log likelihood (separate arrays for compatibility with loo)
  vector[N_weight] log_lik_weight;
  vector[N_other] log_lik_other;
  ...
  for (n in 1:N_weight) {
    y_weight_rep[n] = student_t_rng(nu[1], f_weight[n], sigma[1]);
    log_lik_weight[n] = student_t_lpdf(y_weight[n] | nu[1], f_weight[n], sigma[1]);
  }
  for (n in 1:N_other) {
    y_other_rep[n] = student_t_rng(nu[2], f_other[n], sigma[2]);
    log_lik_other[n] = student_t_lpdf(y_other[n] | nu[2], f_other[n], sigma[2]);
  }
