/*
 * Improved cross-lagged Gaussian Process model with cumulative lag effects.
 * Weight depends on average of lagged workouts over a window:
 *   weight(t) = β·(1/K) Σ_{k=1}^K workouts(t-τ_k) + weight_intrinsic(t)
 ...
 *   where:
 *     f_weight(t) = β·(1/K) Σ_{k=1}^K f_workout(t-τ_k) + g_weight(t)
 ...
 *
 * τ_k are fixed lags specified as data (in scaled time units).
 */
...
data {
  ...
  int<lower=1> K;                     // Number of lags in cumulative window
  array[K] real lag_scaled;           // τ_k in scaled time units (positive, increasing)
...
parameters {
  ...
  // Causal effect: workouts → weight (average effect over lag window)
  real beta;                         // Effect size (weight ~ beta * average lagged workouts)
...
transformed parameters {
  ...
  // Weight latent function with cumulative lagged workout effect
  vector[N_weight] f_weight;
  {
    ...
    for (i in 1:N_weight) {
      real t = t_weight[i];
      real cumulative_effect = 0.0;
      int valid_lags = 0;

      // Sum over all specified lags
      for (k in 1:K) {
        real t_lag = t - lag_scaled[k];
        if (t_lag >= 0 && t_lag <= 1) {
          ...
          cumulative_effect += K_lag_u * a_workout;
          valid_lags += 1;
        }
      }

      // Average effect over valid lags (if any)
      real avg_workout_lag = 0.0;
      if (valid_lags > 0) {
        avg_workout_lag = cumulative_effect / valid_lags;
      }
      ...
      // Combined: average lagged workout effect + intrinsic dynamics
      f_weight[i] = beta * avg_workout_lag + f_weight_intrinsic;
    }
  }
}
...
generated quantities {
  ...
  if (N_pred > 0) {
    ...
    // Weight predictions (need to account for cumulative lag)
    ...
    for (i in 1:N_pred) {
      ...
      // Weight prediction: beta * average lagged workout + intrinsic
      real f_weight_pred = 0;
      for (j in 1:M) {
        f_weight_pred += K_pred_u_weight[i, j] * a_weight[j];
      }
      f_weight_pred += beta * avg_workout_lag;
