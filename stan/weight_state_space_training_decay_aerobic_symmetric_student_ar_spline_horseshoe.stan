 ...
 * HORSESHOE PRIOR on AR(1) innovation scale to regularize AR component.
 ...
 * AR(1) process to model temporal correlation in residuals
 * Daily spline to capture intraday weight variations
 * Horseshoe prior on sigma_epsilon to shrink AR component toward zero
...
parameters {
  ...
  // AR(1) process parameters with horseshoe prior on innovation scale
  real<lower=-1, upper=1> rho;        // autocorrelation coefficient
  real<lower=0> sigma_epsilon_raw;    // raw innovation standard deviation (horseshoe)
  real<lower=0> tau;                  // global shrinkage parameter (horseshoe)
  vector<lower=0>[N_weight] lambda;   // local shrinkage parameters (horseshoe)
...
transformed parameters {
  ...
  // AR(1) innovations (scaled by horseshoe-shrunken sigma_epsilon)
  vector[N_weight] epsilon;
  real<lower=0> sigma_epsilon = tau * sigma_epsilon_raw;
  ...
  // AR(1) process: epsilon[i] = rho * epsilon[i-1] + sigma_epsilon * lambda[i] * epsilon_raw[i]
  // Horseshoe: sigma_epsilon * lambda[i] provides local shrinkage
  // For the first observation, assume stationary distribution
  epsilon[1] = sigma_epsilon * lambda[1] / sqrt(1 - rho^2) * epsilon_raw[1];

  for (i in 2:N_weight) {
    epsilon[i] = rho * epsilon[i-1] + sigma_epsilon * lambda[i] * epsilon_raw[i];
  }
}
...
model {
  ...
  // More informative to encourage lower autocorrelation
  rho ~ normal(0, 0.3);

  // Horseshoe prior for AR(1) innovation scale
  sigma_epsilon_raw ~ std_normal();       // Half-Cauchy(0,1) via parameter expansion
  tau ~ cauchy(0, 0.1);                   // Global shrinkage - smaller scale to shrink more
  lambda ~ cauchy(0, 1);                  // Local shrinkage parameters
  ...
  for (i in 1:N_weight) {
    ...
    y_weight[i] ~ student_t(nu, mu_total, sigma_epsilon * lambda[i]);
  }
}
...
generated quantities {
  ...
  vector[N_weight] lambda_stored = lambda;
  real tau_stored = tau;
  ...
  for (i in 1:N_weight) {
    ...
    y_weight_rep[i] = student_t_rng(nu, mu_pred, sigma_epsilon * lambda[i]);
    log_lik_weight[i] = student_t_lpdf(y_weight[i] | nu, mu_pred, sigma_epsilon * lambda[i]);
  }
  ...
  for (t in 1:D) {
    for (h in 1:H) {
      ...
      y_pred_all_days[t][h] = student_t_rng(nu, mu_pred_all_days[t][h], sigma_epsilon);
    }
  }
}