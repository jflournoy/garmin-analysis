#!/usr/bin/env python3
"""Test optimized cyclic model with cov_exp_quad, cov_periodic, and sparse GP options."""
import sys
...
def test_optimized_model_full_gp():
    """Test that optimized cyclic model with full GP (use_sparse=False) works."""
    print("Testing optimized cyclic model (full GP)...")
    ...
    try:
        ...
        print("✓ Optimized cyclic model (full GP) fitted successfully")
        ...
        required_params = ['alpha_trend', 'rho_trend', 'alpha_daily', 'rho_daily', 'sigma']
        ...
        if 'daily_amplitude' in idata.posterior:
            da_mean = idata.posterior['daily_amplitude'].mean().item()
            print(f"  daily_amplitude mean: {da_mean:.4f}")
        return True
...
def test_numerical_equivalence():
    """Compare original cyclic and optimized cyclic full GP models for numerical equivalence."""
    print("\n" + "="*60)
    print("Testing numerical equivalence: original cyclic vs optimized cyclic (full GP)")
    print("="*60)
    ...
    print("Fitting original cyclic model (manual loops)...")
    try:
        fit_orig, idata_orig, df_orig, stan_data_orig = fit_weight_model_cyclic(
            chains=chains,
        ...
        print("✓ Original cyclic model fitted")
    except Exception as e:
        print(f"✗ Original cyclic model failed: {e}")
        return False

    print("Fitting optimized cyclic model (full GP)...")
    try:
        fit_opt, idata_opt, df_opt, stan_data_opt = fit_weight_model_cyclic_optimized(
            chains=chains,
            iter_warmup=iter_warmup,
            iter_sampling=iter_sampling,
            use_sparse=False,
        ...
        print("✓ Optimized cyclic model fitted")
    except Exception as e:
        print(f"✗ Optimized cyclic model failed: {e}")
        return False
    ...
    comparisons = []
    for param in ['alpha_trend', 'rho_trend', 'alpha_daily', 'rho_daily', 'sigma']:
...
def test_inducing_point_methods():
    ...
    for method in methods:
        ...
        try:
            ...
            print(f"    period_daily: {stan_data.get('period_daily', 'N/A'):.6f}")
...
def main():
    """Run all tests."""
    print("="*70)
    print("OPTIMIZED CYCLIC MODEL TEST SUITE")
    print("="*70)
