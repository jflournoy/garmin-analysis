#!/usr/bin/env python3
"""Test optimized original model with cov_exp_quad and sparse GP options."""
import sys
...
def test_optimized_model_full_gp():
    ...
    print("Testing optimized original model (full GP)...")
    print("Parameters: chains=1, iter_warmup=50, iter_sampling=50")
    try:
        fit, idata, df, stan_data = fit_weight_model_optimized(
            chains=1,
            iter_warmup=50,
            iter_sampling=50,
        ...
        print("✓ Optimized original model (full GP) fitted successfully")
...
def test_numerical_equivalence():
    ...
    print("Fitting original model (manual loops)...")
    try:
    ...
    print("Fitting optimized original model (full GP)...")
    try:
        fit_opt, idata_opt, df_opt, stan_data_opt = fit_weight_model_optimized(
            chains=chains,
            iter_warmup=iter_warmup,
            iter_sampling=iter_sampling,
            use_sparse=False,
...
def main():
    """Run all tests."""
    print("="*70)
    print("OPTIMIZED ORIGINAL MODEL TEST SUITE")
    print("="*70)
