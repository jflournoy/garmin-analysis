#!/usr/bin/env python3
"""Run four-fitness model and save full inference data including fitness states.

This is a modified version of analyze_four_fitness_model.py that saves
the complete InferenceData object for time series analysis.
"""
...
def run_four_fitness_with_states():
    """Run four-fitness model and save full results."""
    print("\n" + "=" * 70)
    print("RUNNING FOUR-FITNESS MODEL WITH STATE SAVING")
    print("=" * 70)
    ...
    # Fit model
    print("\n" + "=" * 70)
    print("FITTING FOUR-FITNESS MODEL")
    print("=" * 70)
    ...
    # Save diagnostics
    diagnostics = {
        'divergences': getattr(fit, 'divergences', []),
        'max_treedepths': getattr(fit, 'max_treedepths', []),
        'step_size': getattr(fit, 'step_size', []),
        'accept_stat': getattr(fit, 'accept_stat', []),
    }
    ...
    if 'gamma_a_short' in idata.posterior:
        ...
        print(f"  Gamma aerobic short-term: {gamma_a_short:.3f} (negative = fat loss)")

    if 'gamma_s_short' in idata.posterior:
        gamma_s_short = idata.posterior['gamma_s_short'].values.mean()
        print(f"  Gamma strength short-term: {gamma_s_short:.3f} (positive = inflammation/water)")

    if 'gamma_a_long' in idata.posterior:
        gamma_a_long = idata.posterior['gamma_a_long'].values.mean()
        print(f"  Gamma aerobic long-term: {gamma_a_long:.3f} (negative = fat loss)")

    if 'gamma_s_long' in idata.posterior:
        gamma_s_long = idata.posterior['gamma_s_long'].values.mean()
        print(f"  Gamma strength long-term: {gamma_s_long:.3f} (positive = muscle gain)")

    # Check if fitness states are available
    fitness_vars = [
        'fitness_a_short_stored', 'fitness_s_short_stored',
        'fitness_a_long_stored', 'fitness_s_long_stored'
    ]

    available_fitness = [v for v in fitness_vars if v in idata.posterior]
    print(f"\n  Fitness states available: {len(available_fitness)}/{len(fitness_vars)}")

    for var in available_fitness:
        samples = idata.posterior[var].values
        print(f"    {var}: shape {samples.shape}")

    print("\n" + "=" * 70)
    print("ANALYSIS COMPLETE")
    print("=" * 70)
    print(f"Full results saved to: {output_dir}")
    print(f"\nTo visualize time series, run:")
    print(f"  uv run python visualize_fitness_time_series.py")


if __name__ == "__main__":
    run_four_fitness_with_states()